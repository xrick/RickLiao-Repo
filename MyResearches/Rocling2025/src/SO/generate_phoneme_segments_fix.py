# generate_phoneme_segments_fixed.py
import os
import glob
...
def load_audio_safe(audio_path: str, target_sr: int = 16000):
    """安全載入音頻檔案，避免 torchcodec 問題"""
    try:
        # 方法1：嘗試使用 torchaudio
        import torchaudio
        ...
        if sr != target_sr:
            audio = torchaudio.functional.resample(
                torch.tensor(audio).unsqueeze(0), sr, target_sr
            ).squeeze().numpy()
        return audio
    except ImportError:
        pass
    ...
    # 如果所有方法都失敗，返回錯誤
    raise RuntimeError("無法載入音頻檔案，請安裝 torchaudio、librosa 或 scipy")
    ...
    def add_phoneme_segments(example):
        ...
        try:
            ...
            if isinstance(audio_path, str):
                # 使用安全載入方法
                audio = load_audio_safe(audio_path, target_sr=16000)
            else:
...
#     parser.add_argument("--dataset_path", required=True, help="原始資料集路徑")
#     parser.add_argument("--output_path", required=True, help="輸出資料集路徑")
